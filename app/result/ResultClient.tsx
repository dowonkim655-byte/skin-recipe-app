...
export default function ResultClient() {
  ...
  return (
    <main className="min-h-screen bg-cream animate-fadeIn">
      ...
      {selectedIng && (() => {
        const meta = getIngredientMeta(selectedIng.name);
        const amountG = calcAmountG(selectedIng.ratio);
        const cost = meta ? calcIngredientCost(selectedIng.ratio, meta.pricePerGram) : null;
        function openSearch(site: 'coupang' | 'naver') {
          if (!meta) return;
          const q = encodeURIComponent(meta.searchKeyword);
          const url = site === 'coupang'
          ...
          >
            ...
            >
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <h2 className="font-bold text-lg text-text-primary leading-tight">{selectedIng.name}</h2>
                  <span className="inline-block mt-1 text-xs font-bold px-2.5 py-1 rounded-full text-white"
                        style={{ backgroundColor: '#c4a882' }}>
                    {selectedIng.ratio}
                  </span>
                </div>
              ...
              <div className="bg-rose-50 rounded-2xl p-3 mb-4">
                <p className="text-xs font-semibold mb-1" style={{ color: '#b97070' }}>이 레시피에서의 역할</p>
                <p className="text-sm text-text-secondary leading-relaxed">{selectedIng.benefit}</p>
              </div>

              {/* Beginner desc */}
              {meta && (
                <div className="mb-4">
                  <p className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">성분 설명</p>
              ...
              <div className="flex gap-2 mb-4">
                ...
                {cost !== null && (
                  <div className="bg-rose-50 rounded-xl px-3 py-2 flex-1">
                    <p className="text-xs text-text-muted mb-0.5">예상 원가</p>
                    <p className="text-sm font-bold" style={{ color: '#b97070' }}>약 {cost.toLocaleString()}원</p>
                  </div>
                )}
                {meta && (
